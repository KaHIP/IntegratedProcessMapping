...
EdgeWeight quotient_graph_refinement::perform_a_two_way_refinement(PartitionConfig & config, 
        ...
        bool only_one_block_is_overloaded = false;
////////////////////////////////////////////////////////////////////////////////        
        if (config.adapt_bal && config.initial_bipartitioning) {
                int lhs_upper_bound = config.target_weights[lhs];
                int rhs_upper_bound = config.target_weights[rhs];

                only_one_block_is_overloaded = boundary.getBlockWeight(lhs) > lhs_upper_bound;
                only_one_block_is_overloaded = only_one_block_is_overloaded 
                                        || boundary.getBlockWeight(rhs) > rhs_upper_bound;
                only_one_block_is_overloaded = only_one_block_is_overloaded && 
                        (boundary.getBlockWeight(lhs) <= lhs_upper_bound || 
                        boundary.getBlockWeight(rhs) <= rhs_upper_bound);
        }
////////////////////////////////////////////////////////////////////////////////        
        else {

...
double quality_metrics::Consistency(graph_access& G, complete_boundary & boundary) {
        std::vector<PartitionID> part_weights(G.get_partition_count(), 0);
        ...
        forall_blocks(G, p) {
                double cur = part_weights[p];
                if (cur != boundary.getBlockWeight(p)) {
                        std::cout << "PARTITION " << p << ". Real: " << cur << ", in Boundary: " << 
                                boundary.getBlockWeight(p) << "\n\n"; 
                        return 0;
                }
        } endfor
...
NodeWeight quality_metrics::total_qap_noquotient(graph_access & G, int * partition_map, matrix & D, std::vector< NodeID > & rank_assign) {
        NodeWeight total_volume = 0;
        forall_nodes(G, node) {
                PartitionID my_block  = partition_map[node];
                forall_out_edges(G, e, node) {
                        NodeID target           = G.getEdgeTarget(e);
                        PartitionID cur_block   = partition_map[target];
                        if (my_block == cur_block)
                                continue;
                        NodeWeight comm_vol     = G.getEdgeWeight(e);
                        NodeID perm_rank_node   = rank_assign[my_block];
                        NodeID perm_rank_target = rank_assign[cur_block];
...
NodeWeight quality_metrics::total_qap_noquotient(graph_access & G, matrix & D, std::vector< NodeID > & rank_assign) {
        NodeWeight total_volume = 0;
        forall_nodes(G, node) {

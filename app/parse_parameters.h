...
#elif defined MODE_SPMXV_TRANSLATION 
                k, 
                input_partition,
                filename_output,
#elif defined MODE_STRONGMAP
                k, 
                imbalance,  
...
/*         cfg.strong(partition_config); */
...
#ifdef MODE_STRONGMAP
        cfg.strongmap(partition_config);
#endif
#ifdef MODE_ECOMAP
        cfg.ecomap(partition_config);
#endif
#ifdef MODE_FASTMAP
        cfg.fastmap(partition_config);
#endif
#ifdef MODE_FASTESTMAP
        cfg.fastestmap(partition_config);
#endif
#ifdef MODE_SSOCIALMAP
        cfg.ssocialmap(partition_config);
#endif
#ifdef MODE_ESOCIALMAP
        cfg.esocialmap(partition_config);
#endif
#ifdef MODE_FSOCIALMAP
        cfg.fsocialmap(partition_config);
#endif
#ifdef MODE_FFSOCIALMAP
        cfg.ffsocialmap(partition_config);
#endif
	...
	if(mh_enable_tabu_search->count > 0) {
        ...
        if (qap_quotient_ref->count > 0) {
                ...
                if(!integrated_mapping->count) {
                        std::cout <<  "--qap_quotient_ref only works together with the --integrated_mapping flag."  << std::endl;
                        exit(0);
                }
        }
        ...
        if (qap_bquotient_ref->count > 0) {
                ...
                if(!integrated_mapping->count) {
                        std::cout <<  "--qap_bquotient_ref only works together with the --integrated_mapping flag."  << std::endl;
                        exit(0);
                }
        }
        ...
        if (qap_0quotient_ref->count > 0) {
                ...
                if(!integrated_mapping->count) {
                        std::cout <<  "--qap_0quotient_ref only works together with the --integrated_mapping flag."  << std::endl;
                        exit(0);
                }
        }
        ...
        if (quotient_more_mem->count > 0) {
                ...
                if(!qap_0quotient_ref->count && !qap_bquotient_ref->count  && !qap_quotient_ref->count ) {
                        std::cout <<  "--quotient_more_mem only works together with QAP quotient graph refinement."  << std::endl;
                        exit(0);
                }
        }

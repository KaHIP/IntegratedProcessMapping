...
EdgeWeight kway_graph_refinement_core::single_kway_refinement_round_internal(PartitionConfig & config, 
        ...
        init_queue_with_boundary(config, boundary, G, start_nodes, queue, moved_idx); 
        ...
        for(number_of_swaps = 0, movements = 0; movements < max_number_of_swaps; movements++, number_of_swaps++) {
                ...
                // if ( (config.qap_bmultitry_kway_fm || config.qap_multitry_kway_fm || config.qap_kway_fm) && 
                //         config.initial_partitioning_type == INITIAL_PARTITIONING_RECPARTITION ) {
                //         ASSERT_EQ(gain, commons->compute_gain_map(config, boundary, G, node, maxgainer, ext_degree));
                // } else {
                ...
                bool successfull;
                successfull = move_node(config, G, node, moved_idx, queue, boundary, &gain);


// quality_metrics qm;
// double balance = qm.Consistency(G, boundary);
...
void kway_graph_refinement_core::init_queue_with_boundary(PartitionConfig & config,
                complete_boundary & boundary,
                graph_access & G,
                ...
                if( moved_idx.find(node) == moved_idx.end() ) {
                        ...
                        if ( (config.qap_bmultitry_kway_fm || config.qap_multitry_kway_fm || config.qap_kway_fm 
                              ...
                              config.initial_partitioning_type == INITIAL_PARTITIONING_RECPARTITION ) {
                                ...
                                //         // int tmp = commons->compute_gain_map(config, boundary, G, node, max_gainer, ext_degree, false);    
                                //         gain = commons->best_delta_gain(config, boundary, G, node, max_gainer, ext_degree, false);
                                //         // if (gain != tmp) {
                                //         //         std::cout << "ERRO 3 (Marcelo)\n";
                                //         // }
                                // } else {
                                        gain = commons->compute_gain_map(config, boundary, G, node, max_gainer, ext_degree, false);
                                // }
                        } else {
                                gain = commons->compute_gain(G, node, max_gainer, ext_degree);
                        }

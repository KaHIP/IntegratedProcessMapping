...
graph_access* graph_hierarchy::pop_finer_and_project(PartitionConfig & config, bool delta_gains=false) {
        ...
        std::vector<std::vector<NodeID>>* map_coarser_finer;
        std::map<NodeID,std::map<PartitionID,DELTA>>* new_delta;
        ...
        // if (delta_gains) {
        //         map_coarser_finer = new std::vector<std::vector<NodeID>>(cRef.number_of_nodes(), std::vector<NodeID>());
        //         new_delta = new std::map<NodeID,std::map<PartitionID,DELTA>>();
        // }
        ...
        forall_nodes(fRef, n) {
                ...
                // if (delta_gains) {
                //         (*map_coarser_finer)[coarser_node].push_back(n);
                // }

                fRef.setPartitionIndex(n, coarser_partition_id);
        } endfor

        // /////// Do here the transition for delta gain and cache gain (Marcelo)
        // if (delta_gains) {
        //         forall_nodes(cRef, a) {
        //                 forall_out_edges(cRef, edge, a) {
        //                         for (int v = 0; v < (*map_coarser_finer)[a].size(); v++) {
        //                                 NodeID finer_node     = (*map_coarser_finer)[a][v];
        //                                 PartitionID par_finer = cRef.getPartitionIndex(finer_node);
        //                         }
        //                 } endfor
        //         } endfor
        // } 

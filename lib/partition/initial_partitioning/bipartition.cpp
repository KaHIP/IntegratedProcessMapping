...
void bipartition::initial_partition( PartitionConfig & config, 
        ...
        bool feasible = false;
        bool switch_feas = false;

        int count_adapt_bal=0;

        for( unsigned i = 0; i < iterations || (config.adapt_bal && !feasible && (count_adapt_bal++ < 10)); i++) {
                ...
                if (!config.adapt_bal) {
                        if(curcut < best_cut || (curcut == best_cut && lhs_overload + rhs_overload < best_load) ) {
                                //store it
                                best_cut  = curcut;
                        ...
                        if( (!feasible && (switch_feas || curcut < best_cut || 
                                (curcut == best_cut && lhs_overload + rhs_overload < best_load)))
                                || (feasible && (curcut < best_cut && switch_feas)) ) {

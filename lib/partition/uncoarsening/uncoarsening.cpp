...
int uncoarsening::perform_uncoarsening_cut(PartitionConfig & config, graph_hierarchy & hierarchy) {
        ...
        if(!config.label_propagation_refinement || (config.integrated_mapping && config.initial_partitioning_type == INITIAL_PARTITIONING_RECPARTITION)) {
        ...
        if(!config.label_propagation_refinement || config.integrated_mapping) {
...
// if (!cfg.adapt_bal || cfg.initial_partitioning_type == INITIAL_PARTITIONING_RECPARTITION) {
        // quality_metrics qm;
        // int qap;
        // if (cfg.integrated_mapping && cfg.initial_partitioning_type == INITIAL_PARTITIONING_RECPARTITION) {
        //         qap = qm.total_qap_noquotient(*coarsest, *D, *perm_rank );
        //         std::cout <<  "LOCAL SEARCH (BEFORE)  - quadratic assignment objective J(C,D,Pi') = " << qap << std::endl;
        // }
        improvement += (int)refine->perform_refinement(cfg, *coarsest, *coarser_boundary);
        // if (cfg.integrated_mapping && cfg.initial_partitioning_type == INITIAL_PARTITIONING_RECPARTITION) {
        //         qap = qm.total_qap_noquotient(*coarsest, *D, *perm_rank );
        //         std::cout <<  "LOCAL SEARCH (AFTER)   - quadratic assignment objective J(C,D,Pi') = " << qap << std::endl;
        // }
// }
        ...
        while(!hierarchy.isEmpty()) {
                ...
                if (cfg.initial_partitioning_type == INITIAL_PARTITIONING_RECPARTITION) {
...
//  if (!cfg.adapt_bal || cfg.initial_partitioning_type == INITIAL_PARTITIONING_RECPARTITION) {               
                // if (cfg.integrated_mapping && cfg.initial_partitioning_type == INITIAL_PARTITIONING_RECPARTITION) {
                //         int qap = qm.total_qap_noquotient(*G, *D, *perm_rank );
                //         std::cout <<  "LOCAL SEARCH (BEFORE)  - quadratic assignment objective J(C,D,Pi') = " << qap << std::endl;
                // }
...
//  }
                ...
                if(config.use_balance_singletons && !config.label_propagation_refinement) {
                        if (config.adapt_bal && config.initial_partitioning_type != INITIAL_PARTITIONING_RECPARTITION) {
...
int uncoarsening::perform_uncoarsening_nodeseparator(PartitionConfig & config, graph_hierarchy & hierarchy) {
        ...
        graph_access* to_delete = NULL;
        while(!hierarchy.isEmpty()) {
                graph_access* G = hierarchy.pop_finer_and_project(config);

...
inline bool kway_graph_refinement_core::move_node(PartitionConfig & config, 
        ...
        if ( (config.qap_bmultitry_kway_fm || config.qap_multitry_kway_fm || config.qap_kway_fm || config.qap_bkway_fm) && 
              config.initial_partitioning_type == INITIAL_PARTITIONING_RECPARTITION ) {
                // if (config.use_delta_gains) {
                //         // int tmp = commons->compute_gain_map(config, boundary, G, node, to, node_ext_deg, false);
                //         *my_gain = commons->best_delta_gain(config, boundary, G, node, to, node_ext_deg, false);
                //         // if ((*my_gain) != tmp) {
                //         //         std::cout << "ERRO 5 (Marcelo)\n";
                //         // }
                // } else {
                        *my_gain = commons->compute_gain_map(config, boundary, G, node, to, node_ext_deg, false);
                // }
        } else {
        ...
        if (to == INVALID_PARTITION || boundary.getBlockWeight(to) + this_nodes_weight > config.upper_bound_partition) {
                return false;
        }
        ...
        //         } 
        //         // else {
        //         //         commons->ignore_delta_gain(config, G, node, -1);
        //         // }
        // }
        ...
        forall_out_edges(G, e, node) {
                ...
                if ( (config.qap_bmultitry_kway_fm || config.qap_multitry_kway_fm || config.qap_kway_fm || config.qap_bkway_fm) && 
                      config.initial_partitioning_type == INITIAL_PARTITIONING_RECPARTITION ) {
                        // if (config.use_delta_gains) {
                        //         // int tmp = commons->compute_gain_map(config, boundary, G, target, targets_max_gainer, ext_degree, false);
                        //         gain = commons->best_delta_gain(config, boundary, G, target, targets_max_gainer, ext_degree, false);
                        //         // if (gain != tmp) {
                        //         //         std::cout << "ERRO 4 (Marcelo)\n";
                        //         // }
                        // } else {
                                gain = commons->compute_gain_map(config, boundary, G, target, targets_max_gainer, ext_degree, false);
                        // }
                } else {
                        gain = commons->compute_gain(G, target, targets_max_gainer, ext_degree);
                }

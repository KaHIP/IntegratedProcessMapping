        ...
        while(!todolist.empty()) {
                ...
                if ( (config.qap_bmultitry_kway_fm || config.qap_multitry_kway_fm || config.qap_kway_fm 
                      ...
                      config.initial_partitioning_type == INITIAL_PARTITIONING_RECPARTITION ) {
                        ...
                        //         // int gain = commons->compute_gain_map(config, boundary, G, node, maxgainer, extdeg, false);
                        //         int tmp  = commons->best_delta_gain(config, boundary, G, node, maxgainer, extdeg, false);
                        //         // if (gain != tmp) {
                        //         //         std::cout << "ERRO 2 (Marcelo)\n";
                        //         // }
                        // } else {
                                commons->compute_gain_map(config, boundary, G, node, maxgainer, extdeg, false);
                        // }
                } else {
                                ...
                                forall_out_edges(G, e, node) {
                                        ...
                                        if(moved_idx.find(target) == moved_idx.end()) {
                                                ...
                                                if ( (config.qap_bmultitry_kway_fm || config.qap_multitry_kway_fm || config.qap_kway_fm
                                                      ...
                                                      config.initial_partitioning_type == INITIAL_PARTITIONING_RECPARTITION ) {
                                                        // if (config.use_delta_gains) {
                                                        //         // int gain = commons->compute_gain_map(config, boundary, G, target, maxgainer, extdeg, false);
                                                        //         int tmp = commons->best_delta_gain(config, boundary, G, target, maxgainer, extdeg, false);
                                                        //         // if (gain != tmp) {
                                                        //         //         std::cout << "ERRO 1 (Marcelo)\n";
                                                        //         // }
                                                        // } else {
                                                                commons->compute_gain_map(config, boundary, G, target, maxgainer, extdeg, false);
                                                        // }
                                                } else {

...
EdgeWeight two_way_fm::perform_refinement(PartitionConfig & cfg, 
        ...
        quality_metrics qm;
        if(!((config.qap_quotient_ref || config.qap_bquotient_ref) 
...
void two_way_fm::move_node(PartitionConfig & config, 
                           graph_access & G,
                           NodeID & node,
                           vertex_moved_hashtable & moved_idx,
        ...
        // if ((config.qap_quotient_ref || config.qap_bquotient_ref) 
        //     && config.initial_partitioning_type == INITIAL_PARTITIONING_RECPARTITION && config.use_delta_gains) {
        //         // commons->move_delta_gain (config, boundary, G, node, from, to);       
        //         commons->move_delta_degrees (config, boundary, G, node, from, to);       
        // }

        EdgeWeight int_degree_node = 0;
        EdgeWeight ext_degree_node = 0;
        bool difficult_update = false;
        if ((config.qap_quotient_ref || config.qap_bquotient_ref) 
                && config.initial_partitioning_type == INITIAL_PARTITIONING_RECPARTITION) {
                bool has_ext_edge=false;
                // if (config.use_delta_gains) {
                //         EdgeWeight int_tmp = 0;
                //         EdgeWeight ext_tmp = 0;
                //         bool has_tmp=false;
                //         // int gain = int_ext_degree_map(config, G, node, to, from, int_tmp, ext_tmp, has_tmp);
                //         difficult_update = int_ext_degree_delta(config, G, node, to, from, int_degree_node, ext_degree_node, has_ext_edge);
                //         // if (gain != difficult_update || int_tmp!=int_degree_node || ext_tmp!=ext_degree_node || has_tmp!= has_ext_edge) {
                //         //         std::cout << "ERRO 10 (Marcelo)\n";
                //         // }
                // } else {
                //         difficult_update = int_ext_degree_map(config, G, node, to, from, int_degree_node, ext_degree_node, has_ext_edge);
                // }
                difficult_update = int_ext_degree_map(config, G, node, to, from, int_degree_node, ext_degree_node, has_ext_edge);
                ...
                difficult_update = int_ext_degree(G, node, to, from, int_degree_node, ext_degree_node);
        ...
        forall_out_edges(G, e, node) {
                ...
                if ((config.qap_quotient_ref || config.qap_bquotient_ref) 
                        ...
                        //         EdgeWeight ext_tmp = 0;
                        //         bool has_tmp=false;
                        //         // int gain = int_ext_degree_map(config, G, target, targets_partition, other_partition, int_tmp, ext_tmp, has_tmp);
                        //         int test = int_ext_degree_delta(config, G, target, targets_partition, other_partition, int_degree, ext_degree, has_ext_edge);
                        //         // if (gain != test || int_tmp!=int_degree || ext_tmp!=ext_degree || has_tmp!= has_ext_edge) {
                        //         //         std::cout << "ERRO 11 (Marcelo)\n";
                        //         // }
                        // } else {
                        //         int_ext_degree_map(config, G, target, targets_partition, other_partition, int_degree, ext_degree, has_ext_edge); 
                        // }
                        int_ext_degree_map(config, G, target, targets_partition, other_partition, int_degree, ext_degree, has_ext_edge); 
                        gain = int_degree - ext_degree;
...
void two_way_fm::move_node_back(PartitionConfig & config, 
                                graph_access & G,
                                NodeID & node,
                                vertex_moved_hashtable & moved_idx,
                                refinement_pq * from_queue,
        ...
        if ((config.qap_quotient_ref || config.qap_bquotient_ref) 
                ...
                //         bool has_tmp=false;
                //         // int gain = int_ext_degree_map(config, G, node, to, from, int_tmp, ext_tmp, has_tmp);
                //         update_difficult = int_ext_degree_delta(config, G, node, to, from, int_degree_node, ext_degree_node, has_ext_edge);
                //         // if (gain != update_difficult || int_tmp!=int_degree_node || ext_tmp!=ext_degree_node || has_tmp!= has_ext_edge) {
                //         //         std::cout << "ERRO 12 (Marcelo)\n";
                //         // }
                // } else {
                //         update_difficult = int_ext_degree_map(config, G, node, to, from, int_degree_node, ext_degree_node, has_ext_edge);
                // }
                update_difficult = int_ext_degree_map(config, G, node, to, from, int_degree_node, ext_degree_node, has_ext_edge);
                ...
                update_difficult = int_ext_degree(G, node, to, from, int_degree_node, ext_degree_node);
        ...
        forall_out_edges(G, e, node) {
                ...
                if ((config.qap_quotient_ref || config.qap_bquotient_ref) 
                        ...
                        //         bool has_tmp=false;
                        //         // int gain = int_ext_degree_map(config, G, target, targets_partition, other_partition, int_tmp, ext_tmp, has_tmp);
                        //         int tmp = int_ext_degree_delta(config, G, target, targets_partition, other_partition, int_degree, ext_degree, has_ext_edge);
                        //         // if (gain != tmp || int_tmp!=int_degree || ext_tmp!=ext_degree || has_tmp!= has_ext_edge) {
                        //         //         std::cout << "ERRO 13 (Marcelo)\n";
                        //         // }
                        // } else {
                        //         int_ext_degree_map(config, G, target, targets_partition, other_partition, int_degree, ext_degree, has_ext_edge);
                        // }
...
void two_way_fm::init_queue_with_boundary(PartitionConfig & config,
                                          graph_access & G,
        ...
        for( unsigned int i = 0, end = bnd_nodes.size(); i < end; i++) {
                ...
                if ((config.qap_quotient_ref || config.qap_bquotient_ref) 
                        && config.initial_partitioning_type == INITIAL_PARTITIONING_RECPARTITION) {
                        EdgeWeight int_obj_func = 0;
                        EdgeWeight ext_obj_func = 0;
                        bool has_ext_edge=false;
                        ...
                        //         EdgeWeight ext_tmp = 0;
                        //         bool has_tmp=false;
                        //         // int gain = int_ext_degree_map(config, G, cur_bnd_node, partition_of_boundary, other, int_tmp, ext_tmp, has_tmp);
                        //         int tmp = int_ext_degree_delta(config, G, cur_bnd_node, partition_of_boundary, other, int_obj_func, ext_obj_func, has_ext_edge);
                        //         // if (gain != tmp || int_tmp!=int_obj_func || ext_tmp!=ext_obj_func || has_tmp!= has_ext_edge) {
                        //         //         std::cout << "ERRO 14 (Marcelo)\n";
                        //         // }
                        // } else {
                        //         int_ext_degree_map(config, G, cur_bnd_node, partition_of_boundary, other, int_obj_func, ext_obj_func, has_ext_edge);
                        // }
                        int_ext_degree_map(config, G, cur_bnd_node, partition_of_boundary, other, int_obj_func, ext_obj_func, has_ext_edge);

                        Gain gain = int_obj_func - ext_obj_func;

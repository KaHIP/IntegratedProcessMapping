...
void graph_partitioner::perform_recursive_partitioning_kmodel_internal(PartitionConfig & config, 
        ...
        if(remaining_k > 1) {
                ...
                for( PartitionID block = 0; block < num_parts; block++) {
                        ...
                        ge.extract_block( G, Q, block, mapping, partition_weight);
...
void graph_partitioner::perform_partitioning_inner_krec_hierarchy(PartitionConfig & config, const unsigned int seed, 
                                                                        graph_access & G, int* partition_map) {
        m_global_k = config.k;
        m_global_upper_bound = config.upper_bound_partition;
        m_rnd_bal = random_functions::nextDouble(1,2);
        perform_recursive_partitioning_inner_kmodel_internal(config, G, config.group_sizes, seed, partition_map);
        forall_nodes(G, n) {
                partition_map[n] =  G.getPartitionIndex(n);
        } endfor
}

void graph_partitioner::perform_recursive_partitioning_inner_kmodel_internal(PartitionConfig & config, 
                                                                graph_access & G, 
                                                                std::vector< int > group_sizes,
        ...
        if( num_parts == 1 ) {
                if( group_sizes.size() == 1 ) return;
                group_sizes.pop_back();
                return perform_recursive_partitioning_inner_kmodel_internal( config, G, group_sizes, seed, partition_map);
        } 
        ...
        double unit_partition = 1;
        for(std::vector<int>::iterator it = group_sizes.begin(); it != group_sizes.end(); ++it) {
                unit_partition *= *it;
        }
        unit_partition /= group_sizes[group_sizes.size()-1];
        // kpart_config.work_load = config.work_load * unit_partition * group_sizes[group_sizes.size()-1] / config.k;
        kpart_config.upper_bound_partition           = config.upper_bound_partition*unit_partition;
        // kpart_config.kway_adaptive_limits_beta    = log(G.number_of_nodes());
        ...
        if (group_sizes.size() > 1) {
                int ismultisec = 1;
                if (config.adapt_bal) {
                        ismultisec = config.interval_sizes[group_sizes.size()-1];
                }
                partition.initial_partition(kpart_config, seed, G, partition_map, ismultisec);
        } else {
                partition.initial_partition(kpart_config, seed, G, partition_map, 0);
        }
        // perform_partitioning(kpart_config, G);        
        graph_extractor extractor;
        group_sizes.pop_back();
        ...
        if(remaining_k > 1) {
                ...
                for( PartitionID block = 0; block < num_parts; block++) {
                        ...
                        ge.extract_block( G, Q, block, mapping, rec_config.work_load);
                        perform_recursive_partitioning_inner_kmodel_internal(rec_config, Q, group_sizes, seed, partition_map);

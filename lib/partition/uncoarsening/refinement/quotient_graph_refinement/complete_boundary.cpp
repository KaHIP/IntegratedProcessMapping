...
// Balance singletons with heterogeneous loads
void complete_boundary::balance_singletons_het_bal(const PartitionConfig & config, graph_access & G) {
        for( unsigned i = 0; i < m_singletons.size(); i++) {
                NodeWeight max = m_block_infos[0].block_weight - config.target_weights[0];
                PartitionID p  = 0;
                for( unsigned j = 0; j < m_block_infos.size(); j++) {
                        if( m_block_infos[j].block_weight - config.target_weights[j] > max ) {
                                max = m_block_infos[j].block_weight;
                                p = j;
                        }
                }
